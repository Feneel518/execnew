...
import { QuotationType, QuotationWithProductInQuotation } from "@/lib/types";
...
interface QuotationProps {
  quotationData: QuotationWithProductInQuotation;
}

const Quotation: FC<QuotationProps> = ({ quotationData }) => {
  const [perPage, setPerPage] = useState([
    quotationData.ProductInQuotation.length,
  ]);
  ...
  let pages = perPage.map((amount, i) => {
    ...
    return quotationData.ProductInQuotation.slice(offset, offset + amount);
  });
    ...
    <div className="flex flex-col gap-4 print:gap-0">
      ...
      <div className="w-[210mm] h-[297mm] print:size-[A4] bg-white text-black shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px] flex flex-col">
        ...
        <QuotationTermsShort
          ...
          footer={
            <QuotationFooter
              pageIndex={quotationData.ProductInQuotation.length + 1}
              totalLength={quotationData.ProductInQuotation.length + 2}
            ></QuotationFooter>
          }
        ></QuotationTermsShort>
      </div>
      <div className="w-[210mm] h-[297mm] print:size-[A4] bg-white text-black shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px] flex flex-col">
        ...
        <div className="flex flex-col h-full">
          ...
          <QuotationFooter
            pageIndex={quotationData.ProductInQuotation.length + 2}
            totalLength={quotationData.ProductInQuotation.length + 2}
          ></QuotationFooter>
        </div>
      </div>
